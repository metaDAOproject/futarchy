...
export default async function test() {
    ...
    let question: PublicKey = await vaultClient.initializeQuestion(sha256(new TextEncoder().encode("Who's going to win the next election?/TRUMP/HARRIS")), operator.publicKey, 2);
    ...
    await vaultClient.splitTokensIx(question, vault, USDC, new BN(100), 2, bob.publicKey).signers([bob]).rpc();
    ...
    await vaultClient.mergeTokensIx(question, vault, USDC, new BN(20), 2, alice.publicKey).signers([alice]).rpc();
    ...
    await vaultClient.redeemTokensIx(question, vault, USDC, 2, charlie.publicKey).signers([charlie]).rpc();
