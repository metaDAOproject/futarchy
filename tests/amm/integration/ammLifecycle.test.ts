...
export default async function() {
    ...
    let userLpBalance = (await getAccount(this.banksClient, userLpAccount)).amount;
    await ammClient.removeLiquidityIx(amm, META, USDC, new anchor.BN(Number(userLpBalance) / 2), new anchor.BN(0), new anchor.BN(0)).rpc();
    ...
    await ammClient.removeLiquidityIx(amm, META, USDC, new anchor.BN(userLpBalance), new anchor.BN(0), new anchor.BN(0)).rpc();
