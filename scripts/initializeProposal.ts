...
async function main() {
  ...
  const proposalIx = await buildMemoInstruction("HelloWorld, I am Drift. I approve this proposal.");
  ...
  const dao = new PublicKey("5vVCYQHPd8o3pGejYWzKZtnUSdLjXzDZcjZQxiFumXXx");
  const storedDao = await autocratClient.getDao(dao);

  const proposal = await autocratClient.initializeProposal(
    dao,
    "https://hackmd.io/@ansatz/drifut-welcome-the-futarchs",
    ix,
    new BN(22_681 * 1_000_000),
