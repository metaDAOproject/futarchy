...
#[derive(Accounts)]
pub struct InitializeNewConditionalVault<'info> {
    #[account(
        init,
        payer = payer,
    ...
    pub vault: Box<Account<'info, NewConditionalVault>>,
...
impl<'info, 'c: 'info> InitializeNewConditionalVault<'info> {
    pub fn handle(ctx: Context<'_, '_, 'c, 'info, Self>) -> Result<()> {
        ...
        vault.set_inner(NewConditionalVault {
