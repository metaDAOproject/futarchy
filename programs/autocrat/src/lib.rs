...
pub mod autocrat {
    ...
    pub fn finalize_proposal(ctx: Context<FinalizeProposal>) -> Result<()> {
        ...
        let fail_market_twap = calculate_twap(&pass_amm)?;
        let pass_market_twap = calculate_twap(&fail_amm)?;
...
pub struct FinalizeProposal<'info> {
    ...
    pub openbook_twap_pass_market: Box<Account<'info, TWAPMarket>>,
    pub openbook_twap_fail_market: Box<Account<'info, TWAPMarket>>,
